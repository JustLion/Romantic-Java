...
### 4、 Java的四种引用类型？
1. 强引用：只要强引用还在，`GC`永远不会回收掉引用的对象
2. 软引用：还有用非必需的对象，在系统将会发生内存溢出之前，会把这些对象列入回收范围进行二次回收
3. 弱引用：弱引用的对象只能生存在下次`GC`之前，`GC`工作时，无论内存足够都会回收
4. 虚引用：随时都会被回收
...
### 13、 GC分代收集算法 vs 分区收集算法
1. 分代收集算法
    1. 在新生代复制算法：复制少量存活对象的复制成本完成收集
    2. 在老年代标记整理算法：独享存活率高，对已死对象进行“标记-清理”或者“标记-整理”
2. 分区收集算法：将整个堆空间分为连续不同的小区间，每个小区间独立使用独立回收。
...
- 单线程收集器
- stop the world
- Java虚拟机运行在 Client 模式下默认的新生垃圾收集器
2. ParNew 垃圾收集器（Serial+多线程）
- Serial 收集器的多线程版本
- 适用复制算法
- stop the world
- 配置-XX:ParallelGCThreads 参数来限制垃圾收集器的线程数
- 很多 Java 虚拟机在 Server 模式下新生代默认的垃圾收集器
...
- 单线程收集器
- 标记整理算法
- Client 模式下Java 虚拟机默认的老年代垃圾收集器
- Server 模式下在 JDK1.5 之前版本中与新生代的 Parallel Scavenge 收集器搭配使用
- Server 模式下 作为年老代中使用 CMS 收集器的后备垃圾收集方案。
5. Parallel Old 收集器（多线程标记整理算法）
- Parallel Scavenge的年老代版本
- 多线程的标记-整理算法
- 为了在年老代同样提供吞吐量优先的垃圾收集器
6. CMS 收集器（多线程标记清除算法）
- 老年代垃圾收集器
- 获取最短垃圾回收停顿时间
- 标记-清除算法
